...
const steps = [
  ...
  { id: 3, name: "Upload PDF", icon: HiOutlineSquare3Stack3D },
  { id: 4, name: "Options", icon: IoMdOptions },
  { id: 5, name: "Generated Course", icon: FaWandMagicSparkles },
];
...
const CreateCourse = () => {
  ...
  const handlePdfData = (pdfUrl, extractedText) => {
    setFormData(prev => ({
      ...prev,
      uploadPdf: pdfUrl,
      extractedText: extractedText,
    }));
    // Move to next step when PDF is processed
    setStep(3);
  };
  ...
  const isNextEnabled = () => {
    switch (step) {
      ...
      case 2: return formData.uploadPdf !== ""; // PDF upload step
      case 3: return (
        formData.difficulty &&
        formData.duration &&
        formData.deadline && // Add deadline validation
        formData.noOfChp >= 1 &&
        formData.noOfChp <= 5
      );
  ...
  const GenerateChapterContent = async () => {
    ...
    try {
        ...
        const transformCourse = (course) => ({
            ...
            overallDeadline: course["OverallDeadline"],
...
// Function to create assigned course for teacher
const createAssignedCourseForTeacher = async (courseId) => {
    try {
        const assignedCoursePayload = {
            teacher: user._id,  // Assuming user object has _id
            assigns: [],       // No students initially
        ...
        toast.success("Assigned course created for teacher.");
    } catch (error) {
        console.error("Failed to create assigned course for teacher:", error);
        toast.error("Failed to create assigned course for teacher.");
    }
};
  ...
  {step < 4 && (
    <>
      ...
      {step === 2 && (
        <div className="space-y-4">
          <h3 className="text-2xl font-bold text-purple-800 mb-4">Upload Course PDF</h3>
          <PDFUploader onPdfProcessed={handlePdfData} />
        </div>
      )}
  ...
  {step === 4 && apiResponse.courseLayout && Object.keys(apiResponse.courseLayout).length > 0 && (
    <div>
      {(() => {
        const course = apiResponse.courseLayout;
        console.log(course);
          ...
          <div className="space-y-6">
            ...
            <div className="bg-purple-50 p-6 rounded-xl border border-purple-100">
              ...
              <div className="grid grid-cols-2 gap-2 text-gray-700">
                ...
                <p><strong>Overall Deadline:</strong> {course.OverallDeadline}</p>
            ...
            <div>
              ...
              {course.Chapters.map((chapter, chapterIdx) => (
                ...
                >
                  ...
                  <p className="mb-2"><strong>Deadline:</strong> {chapter.Deadline}</p>
                  <div>
