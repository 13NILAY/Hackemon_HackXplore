...
const ChapterDetails = () => {
    ...
    useEffect(() => {
        const checkChapterAssessment = async () => {
            if (!token) return;
            
            try {
                const response = await fetch(
                    `http://localhost:8000/api/assessment/course/${courseId}/chapter/${chapterId}`,
                ...
                if (response.ok) {
                    ...
                    if (bestScore >= 70) {
                        // Chapter assessment passed, unlock next chapter
                        handleNextChapter();
                    }
                }
            } catch (error) {
                console.error('Error checking chapter assessment:', error);
            }
        };
    
        checkChapterAssessment();
                    ...
                    <div className="grid md:grid-cols-2 gap-6 mb-6">
                        <div>
                            ...
                            <div className="flex items-center">
                                ...
                                <p><strong>Duration:</strong> {chapter.duration} minutes</p>
                            </div>
                            <div className="flex items-center">
                                <CalendarDays className="mr-3 text-blue-600" size={24} />
                                <p><strong>Deadline:</strong> {(chapter.deadline).split("T")[0]}</p>
                            </div>
                        </div>
